...
// TokenRef is a token reference referencing the actual source byte slice.
...
func (r TokenRef) Value(source []byte) []byte {
	if r.IndexTail < 0 {
		return nil
	}
	return source[r.IndexTail:r.IndexHead]
}
